...
(function () {
    $(function () {
        ...
        $document
            ...
            .on('change', '.materials select', changeLineColors);
        ...
        $(".company-entity-add .close").on("click", function () {
         ...
         $(".ur-delete").on("click", function () {
            $.getJSON(ajaxPath, "mode=deleteUr&id="+$(this).data('id'), function(jsonResult){
                $this.fadeOut(150);
            });
        });
    ...
    function changeLineColors() {
        $(this).parent().nextAll('.inner').removeClass('active')
            .filter('[data-id=color-modal-' + $(this).val()  + ']').addClass('active');
    }
